    ...
    download(from) {
        return new Promise((resolve, reject) => {
            if (!helpers.isZip(from)) {
                let errMsg = `${from} is poiting to a .zip file...exiting`;
                reject(errMsg);
                return false;
            }
            ...
            if (helpers.isUrl(from)) {
                const transfer = from.startsWith('https') ? require('https') : require('http');
                const req = transfer.request(from);
                ...
                req.on('response', res => {
                    ...
                    res.on('data', chunk => {
                        ...
                        fs.appendFileSync(constants.tmpDownloadFile, chunk)
                    });
